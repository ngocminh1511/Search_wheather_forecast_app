...
# Products we generate tiles for.
...
_MAP_PRODUCTS: dict[str, list[str]] = {
    ...
    # rain_advanced: chỉ có 1 tile product là precip_base (do precip_pipeline
    # sinh trong cùng 1 lượt classify+tile). Trước đây list này khai báo thêm
    # "precip_fx" — KHÔNG có code nào generate nó. Hệ quả:
    #   - orchestrator enqueue 2 job per fff (precip_base + precip_fx)
    #   - task_generate_custom_frame ignore product_name → cả 2 job chạy
    #     cùng generate_precip_frame() trên CÙNG file .npy + cùng chunk dir
    #     → race condition (np.save không atomic, rmtree xóa dữ liệu của
    #     job kia) → chunk corrupt / FileNotFoundError / invalid magic.
    "rain_advanced":          ["precip_base"],
